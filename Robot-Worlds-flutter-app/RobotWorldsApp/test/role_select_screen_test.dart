...
void main() {
  testWidgets('when', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: Selection()));

    // Check for Text Fields and Button.
    expect(find.byKey(const Key('Robot Worlds')), findsOneWidget);
    expect(find.byKey(const Key('Select Player or Admin')), findsOneWidget);

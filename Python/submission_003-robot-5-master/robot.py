...
mazename = 'obstacles'
turtle_check = False
maze_list = ['dark_maze','obstacles','the_maze']
...
for x in sys.argv:
    ...
    if x == 'text':
        import world.text.world as world
    if x in maze_list:
       obstacles = import_helper.dynamic_import("maze." + x)
       mazename = x
...
def valid_command(command):
    ...
    if command_name.lower() == 'replay':
        ...
        if arg1.lower() in valid_sides or arg1 == '':
            return True
    else:
...
MAZERUN - solves the maze to the specified side of the maze, e.g. 'MAZERUN LEFT'
"""

def do_mazerun(robot_name, argument):
    """
    handles the mazerun command, sending parameters for which edge the robot must navigate to, whether it must be 
    done visually or textually and the obstacle list
    """
    coords = (0,200)
    i = 1
    edge = 'top'
    top = argument.lower().find('top') > -1
    bottom = argument.lower().find('bottom') > -1
    left = argument.lower().find('left') > -1
    right = argument.lower().find('right') > -1
    if top:
        coords = (0, 200)
        i = 1
        edge = 'top'
    ...
    if len(obstacle_list) == 1:
        mazerun.simple_solver(robot_name, coords, i, turtle_check, edge)
    else:
        mazerun.BFS_SP(robot_name, coords, i, turtle_check, edge, obstacle_list)
    return True, '\033[A'
...
def handle_command(robot_name, command):
    ...
    print(command_output)
    if len(sys.argv) > 1 and 'turtle' in sys.argv:
...
def robot_start():
    ...
    global position_x, position_y, current_direction_index, history, obstacle_list
    # world.obstacles.generate_obstacles()
    robot_name = get_robot_name()
    output(robot_name, "Hello kiddo!")
    print(robot_name+': Loaded '+mazename+'.')
    if turtle_check:
        world.draw_obstacles()
    if not turtle_check:
        world.print_obstacles(robot_name)
    obstacle_list = obstacles.return_obstacles()

...
public class MapObstacles implements Obstacle{
     ...
     * */
    public MapObstacles(int x, int y) {
        this.bottomLeftX = x;
        this.bottomLeftY = y;
        ConfigReader config = new ConfigReader();
        this.size = Integer.parseInt(config.getObsSize());
    }
    ...
    public boolean blocksPath(Position a, Position b) {
        ...
        if (a.getX() == b.getX()) {
            ...
            for (int y = startY; y <= endY; y++) { //checks from min to max y's if za.co.wethinkcode.server.position is blocked
                if (blocksPosition(new Position(a.getX(), y))) return true;
            }
        } else {
